...
class Archives extends Component {
  ...
  fetchList({ page = 1 }) {
    ...
    axios
      .get('/article', { params: {ordering: '-create_at',page:page}
      })
      .then(res => {
        const list = groupBy(res.results, item => item.create_at.slice(0, 4))
    ...
    return (
      <div className="content-inner-wrapper archives">
        <Spin tip="Loading..." spinning={loading}>
            ...
            {list.map((d, i) => (
                ...
                {d.map(item => (
                  ...
                  <Timeline.Item key={item.id}>
                    <span style={{ fontSize: '13px', marginRight: '16px' }}>{item.create_at.slice(5, 10)}</span>
          ...
          {list.length < total && (
            <BlogPagination current={parseInt(current) || 1} onChange={this.handlePageChange} total={total}  pageSize={10} />
          )}
        </Spin>
      </div>
    )
