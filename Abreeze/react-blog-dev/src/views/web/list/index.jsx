...
class List extends Component {
  ...
  fetchList = ({ page = 1, name, type }) => {
    this.setState({ loading: true })
          // axios
          //     .get(`${type}`, { params: { page, pageSize: 15, name } })
          //     .then(res => {
          //         this.setState({ list: res.results, total: res.count, loading: false })
          //     })
          //     .catch(e => this.setState({ loading: false }))
      if (type === 'categories') {
          axios
              .get(`article?`, { params: { page, pageSize: 15, category:name } })
              .then(res => {
                  this.setState({ list: res.results, total: res.count, loading: false })
              })
              .catch(e => this.setState({ loading: false }))
          ...
          axios
              .get(`article?`, { params: { page, pageSize: 15, tag:name } })
              .then(res => {
                  this.setState({ list: res.results, total: res.count, loading: false })
              })
              .catch(e => this.setState({ loading: false }))
